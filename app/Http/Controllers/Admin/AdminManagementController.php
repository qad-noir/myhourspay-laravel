...
class AdminManagementController extends Controller
{
    ...
    public function deleteUser(Request $request, User $user): RedirectResponse
    {
        ...
        DB::transaction(function () use ($request, $user): void {
            ...
            $user->hoursEntries()->delete();
            $user->ownedWorkspaces()->each(fn (Workspace $workspace) => $this->trashWorkspaceData($workspace));
    ...
    public function restoreUser(Request $request, int $user): RedirectResponse
    {
        ...
        $model->restore();
        $model->ownedWorkspaces()->onlyTrashed()->get()->each(function (Workspace $workspace): void {
            $workspace->restore();
            $workspace->hoursEntries()->onlyTrashed()->restore();
        });
        $model->hoursEntries()->onlyTrashed()->restore();
        $this->audit->record($request, 'user.restored', $model);
    ...
    {
        $model = User::onlyTrashed()->findOrFail($user);
        abort_if($request->user()->is($model), 422);
        $this->audit->record($request, 'user.permanently_deleted', $model, $model->only(['name', 'email']));
        $model->ownedWorkspaces()->withTrashed()->get()->each(fn (Workspace $workspace) => $workspace->forceDelete());
        $model->hoursEntries()->withTrashed()->forceDelete();
        $model->forceDelete();
    ...
    public function storeWorkspace(Request $request): RedirectResponse
    {
        ...
        $workspace = DB::transaction(function () use ($data, $owner): Workspace {
            $workspace = $owner->ownedWorkspaces()->create($this->workspaceValues($data));
            $workspace->users()->attach($owner, ['role' => 'owner', 'position' => $data['position']]);

            return $workspace;
        });
        $this->audit->record($request, 'workspace.created', $workspace);

        return to_route('admin.workspaces.show', $workspace)->with('status', 'Workspace created.');
    }
    ...
    public function deleteWorkspace(Request $request, Workspace $workspace): RedirectResponse
    {
        DB::transaction(function () use ($request, $workspace): void {
            $this->audit->record($request, 'workspace.trashed', $workspace);
            $this->trashWorkspaceData($workspace);
        });
    ...
    public function restoreWorkspace(Request $request, int $workspace): RedirectResponse
    {
        ...
        $model->restore();
        $model->hoursEntries()->onlyTrashed()->restore();
        $this->audit->record($request, 'workspace.restored', $model);

        return back()->with('status', 'Workspace restored.');
    }

    public function forceDeleteWorkspace(Request $request, int $workspace): RedirectResponse
    {
        $model = Workspace::onlyTrashed()->findOrFail($workspace);
        $this->audit->record($request, 'workspace.permanently_deleted', $model, ['name' => $model->name]);
        $model->hoursEntries()->withTrashed()->forceDelete();
        $model->forceDelete();

        return back()->with('status', 'Workspace permanently deleted.');
    }
    ...
    {
        $data = $this->hoursData($request);
        $entry = HoursEntry::query()->create($data);
        $this->audit->record($request, 'hours.created', $entry);

        return to_route('admin.hours.index')->with('status', 'Hours entry created.');
    }

    public function updateHours(Request $request, HoursEntry $hoursEntry): RedirectResponse
    {
        $data = $this->hoursData($request, $hoursEntry);
        $before = $hoursEntry->toArray();
        $hoursEntry->update($data);
        $this->audit->record($request, 'hours.updated', $hoursEntry, $before, $hoursEntry->toArray());

        return to_route('admin.hours.index')->with('status', 'Hours entry updated.');
    }

    public function deleteHours(Request $request, HoursEntry $hoursEntry): RedirectResponse
    {
        $this->audit->record($request, 'hours.trashed', $hoursEntry);
        $hoursEntry->delete();

        return back()->with('status', 'Hours entry moved to trash.');
    }

    public function restoreHours(Request $request, int $hoursEntry): RedirectResponse
    {
        $entry = HoursEntry::onlyTrashed()->findOrFail($hoursEntry);
        $entry->restore();
        $this->audit->record($request, 'hours.restored', $entry);

        return back()->with('status', 'Hours entry restored.');
    }

    public function forceDeleteHours(Request $request, int $hoursEntry): RedirectResponse
    {
        $entry = HoursEntry::onlyTrashed()->findOrFail($hoursEntry);
        $this->audit->record($request, 'hours.permanently_deleted', $entry);
        $entry->forceDelete();

        return back()->with('status', 'Hours entry permanently deleted.');
    }
