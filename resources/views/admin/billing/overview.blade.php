...
<div class="admin-columns"><section class="admin-card"><header><div><h2>Plan distribution</h2><p>Active local subscription items by catalogue plan</p></div></header><div class="admin-audit">@forelse($planDistribution as $plan)<div><strong>{{ $plan->name }}</strong><span>{{ number_format($plan->subscribers) }} subscribers</span></div>@empty<p>No paid plan data yet.</p>@endforelse</div></section><section class="admin-card"><header><div><h2>Top premium capabilities</h2><p>Top 10 · Feature middleware usage over 30 days</p></div><a wire:navigate href="{{ route('admin.billing.capabilities') }}">View all capabilities →</a></header><div class="admin-audit">@forelse($topFeatures as $feature)<div><strong>{{ str($feature->feature_key)->replace('_',' ')->headline() }}</strong><span>{{ number_format($feature->uses) }} uses</span></div>@empty<p>No feature usage recorded yet.</p>@endforelse</div></section></div>
@endsection
