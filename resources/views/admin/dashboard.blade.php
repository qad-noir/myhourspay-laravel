...
<div class="admin-columns">
    <section class="admin-card"><header><div><h2>Newest users</h2><p>Recently created accounts</p></div><a wire:navigate href="{{ route('admin.users.index') }}">View all →</a></header><div class="admin-list">@forelse($recentUsers as $user)<a wire:navigate href="{{ route('admin.users.show',$user) }}"><span class="admin-avatar">{{ str($user->name)->substr(0,1)->upper() }}</span><span><strong>{{ $user->name }}</strong><small>{{ $user->email }}</small></span><i>{{ $user->created_at->diffForHumans() }}</i></a>@empty<p>No users yet.</p>@endforelse</div></section>
