...
<main>
    ...
    <section class="public-container pricing-intro">
        ...
        <nav class="pricing-interval" aria-label="Billing interval">
            <a href="{{ route('pricing', ['interval'=>'monthly']) }}" @if($interval === 'monthly') aria-current="true" @endif>Monthly</a>
            <a href="{{ route('pricing', ['interval'=>'yearly']) }}" @if($interval === 'yearly') aria-current="true" @endif>Yearly</a>
        </nav>
    ...
    <section class="public-container pricing-cards" aria-label="Plan prices">
        @foreach($plans as $plan)
            @php
                $price = $plan->prices->first(fn ($price) => $price->kind === 'base' && $price->interval === $interval);
                $seat = $plan->prices->first(fn ($price) => $price->kind === 'seat' && $price->interval === $interval);
                $free = $plan->key === 'free';
            ...
            <article class="pricing-card {{ $plan->key === 'pro' ? 'pricing-card--pro' : '' }}">
                ...
                <div class="pricing-card__price">
                    ...
                    @elseif($price)<strong>{{ $money($price->amount, $price->currency) }}</strong><span>/ {{ $interval === 'yearly' ? 'year' : 'month' }}</span>
                    @else<strong class="pricing-card__unavailable">Not available</strong><span>For {{ $interval }} billing</span>@endif
                    @if($price && !$free)
                        <small>{{ $price->tax_inclusive ? 'Tax inclusive' : 'Tax calculated at checkout' }}
                            @if($interval === 'yearly') · Equivalent to {{ $money($price->amount / 12, $price->currency) }}/month, billed yearly @endif
                        </small>
                    @elseif($free)
                ...
                @if($plan->key === 'business')
                    <p class="pricing-card__seats">{{ config('billing.business_included_seats') }} seats included.
                        @if($seat) Additional seats: {{ $money($seat->amount, $seat->currency) }}/{{ $interval === 'yearly' ? 'year' : 'month' }} each. {{ $seat->tax_inclusive ? 'Tax inclusive.' : 'Tax calculated at checkout.' }}
