...
@else
    ...
    <div class="pro-record-list tool-payroll-list">@forelse($payrollProfiles as $profile)<article><span class="pro-avatar">{{ str($profile->format)->upper() }}</span><div><strong>{{ $profile->name }}</strong><small>{{ collect($profile->columns)->map(fn($column)=>str($column)->headline())->join(', ') }}</small></div>@if($approvedTimesheetCount > 0)<form method="GET" action="{{ route('business.payroll.download',$profile) }}" class="tool-record-action"><input type="date" name="start" value="{{ old('start') }}" aria-label="Export start date" required><input type="date" name="end" value="{{ old('end') }}" aria-label="Export end date" required><button>Export approved time</button></form>@else<span class="tool-record-status">Awaiting approved time</span>@endif</article>@empty<x-tools.empty-state icon="payroll" title="No export profile" description="Save the columns your payroll system expects before choosing a date range." />@endforelse</div>
